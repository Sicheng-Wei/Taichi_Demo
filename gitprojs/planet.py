# Author: https://github.com/peng-bo/voxel-challenge/blob/main/planet.py
...
def land_color(latitude):
    n = latitude / 15
    return vec3(0.5 - 0.5 * n + 0.145 * n * n - 0.005 * n * n * n, 0.4 - 0.055 * n - 0.04 * n * n + 0.015 * n * n * n,
    ...
    for I in ti.grouped(ti.ndrange((-r, r), (-r, r), (-r, r))):
        ...
        if I.norm() < r:
            ...
            if (h := fbm(i + MANUAL_SEED)) < sea_level:
                ...
                mat, clr = 1, land_color(
                    ti.abs(degrees(v) - 90) + 20 * fbm(vec3(u, v, 10 * h) * 4 + fbm(vec3(u, v, 10 * h) * 4)))
...
def cloud(r, density):
    for I in ti.grouped(ti.ndrange((-r, r), (-r, r), (-r, r))):
        scale = 8
        i = I / scale
        [cyclone(i, normalize(rand(vec3((n >> 0) % 2, (n >> 1) % 2, (n >> 2) % 2))) * r / scale, 3 + ti.randn() * 2) for
         n in range(8)]
        if r - 1 < I.norm() < r:
