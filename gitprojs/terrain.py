# Author: https://github.com/bismarckkk/voxel-challenge/blob/main/main.py
from scene import Scene
import taichi as ti
from taichi.math import *

# biome-生物群落, snow-雪
manual_seed, biome, snow = 2, 'desert', False
...
def createRiver(x: ti.i8, y: ti.i8):
    ...
    while 0 < x < 127 and 0 < y < 127 and count < 128:
        ...
        for u, v in ti.ndrange((x - ti.random() - 1, x + ti.random() + 2), (y - ti.random() - 1, y + ti.random() + 2)):
            ...
            for z in range(zll - 4, zll + 2):
                scene.set_voxel(vec3(u - 64, z, v - 64), 1 if z < zl else 0, vec3(0.12, 0.56, 1) + (ti.random() - 0.5) * 0.15)
...
def createTree(x: ti.i16, y: ti.i16):
    zl = ti.cast(scene.get_voxel(vec3(x - 64, 0, y - 64))[1][0] * 80, ti.i16)
    if biome == 'desert':
        for z in range(zl + 2, int(zl + 4 + ti.random() * 2)):
            scene.set_voxel(vec3(x - 64, z, y - 64), 1, vec3(.22, .37, .06))
...
@ti.kernel
def createTerrain():
    for x, y, z in ti.ndrange(127, 127, 127):
        scene.set_voxel(vec3(x - 64, z - 64, y - 64), 0, vec3(0))
    if True:
        [perlinNoise(it[0], it[1]) for it in terrain]
    for x, y in ti.ndrange(127, 127):
        zl = ti.cast(scene.get_voxel(vec3(x - 64, 0, y - 64))[1][0] * 80, ti.i16)
        for i in range(zl):
            color = vec3(.97, .97, 1) if snow else vec3(0.13, 0.55, 0.13)
            if i < zl - (ti.random() - 0.5) * 2 - 6:
                color = vec3(0.5)
            elif biome == 'desert':
                color = vec3(.96, .87, .7)
            elif zl < 41 and biome == 'island':
                color = vec3(.96, .87, .7)
            elif i < zl - 1:
                color = vec3(0.45, 0.29, 0.07)
...
def fillTemp():
    if biome == 'island':
        for x, y in ti.ndrange(127, 127):
            ...
            if zl >= 42:
                continue
            for z in range(zl, 42):
                scene.set_voxel(vec3(x - 64, z, y - 64), 1, vec3(.07, .49, .95) + ti.random() * 0.1)
...
if biome == 'grass' or biome == 'desert':
    createTrees()
elif biome == 'forest':
    createRivers(), createTrees(), createTrees()
elif biome == 'island':
    createTrees(), createTrees(), createTrees()
fillTemp()
scene.finish()
